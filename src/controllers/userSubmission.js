...
const submitCode =async (req,res)=>{
    try{
        ...
        if(!userId || !code || problemId || !language){ return res.status(400).send("some field are missing"); }
...
//}]
        ...
        for(const test of testResult){
            if(test.status_id==3){ 
                ...
                memory = Math.max(memory,text.memory)
            }
            else{
                if(test.status_id==4){
                    status ='error';
        ...
        res.status(500).send("submitCode in usersubmisison in controllers error"+err)
